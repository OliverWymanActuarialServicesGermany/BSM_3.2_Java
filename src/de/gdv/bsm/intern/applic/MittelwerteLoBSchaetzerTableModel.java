 ...
 */
public class MittelwerteLoBSchaetzerTableModel implements SizedTableModel {
	private TreeMap<Integer,TreeMap<String,TreeMap<String,KennzahlenMittelungLoB>>> mittelwerte;
	 ...
	 *            die Daten
	 */ 
	public MittelwerteLoBSchaetzerTableModel(final TreeMap<Integer,TreeMap<String,TreeMap<String,KennzahlenMittelungLoB>>> mittelwerte) {
		
		this.mittelwerte = mittelwerte;
		alleTitle =	KennzahlenMittelungLoB.getTitle();
	...
	public int getRowCount() {
		return mittelwerte.values().stream().map(e -> e.values()).flatMap(l -> l.stream()).map(k -> k.values()).flatMap(h -> h.stream()).collect(Collectors.toList()).size();
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		final KennzahlenMittelung mw = mittelwerte.values().stream().map(e -> e.values()).flatMap(l -> l.stream()).map(k -> k.values()).flatMap(h -> h.stream()).collect(Collectors.toList()).get(rowIndex);
		// ;
